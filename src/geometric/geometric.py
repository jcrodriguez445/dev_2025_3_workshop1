...
class Geometria:
    ...
    def pendiente_recta(self, x1, y1, x2, y2):
        # Lanza ZeroDivisionError si la línea es vertical, como espera el test
        return (y2 - y1) / (x2 - x1)
    
    def ecuacion_recta(self, x1, y1, x2, y2):
        # Coeficientes que coinciden con el test: Ax + By + C = 0
        A = y2 - y1
        B = x1 - x2
        C = x2*y1 - x1*y2
        return (A, B, C)
    ...
    def area_poligono_regular(self, num_lados, lado, apotema):
        # Fórmula correcta: (perímetro * apotema) / 2
